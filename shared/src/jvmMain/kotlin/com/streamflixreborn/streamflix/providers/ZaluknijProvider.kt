...
object ZaluknijProvider : Provider {
    ...
    private var storedClearanceCookie: String? = null
    ...
    private fun getResolver(): HeadlessBrowserResolver {
        return webViewResolver ?: HeadlessBrowserResolver().also {
            webViewResolver = it
        }
    }
    ...
    private fun parseTiles(container: Element): List<ListItem> {
    ...
    private fun parseHomeItems(header: Element, container: Element): List<ListItem> {
        return when {
    ...
    private fun parseHomeMovies(container: Element): List<ListItem> {
    ...
    private fun parseSearchResults(document: Document): List<ListItem> {
    ...
    private fun parseHomeEpisodes(container: Element): List<ListItem> {
    ...
    private fun itemKey(item: ListItem): String {
        return when (item) {
    ...
    private suspend fun getDocument(url: String): Document {
        return try {
            ...
            val html = providerMutex.withLock { getResolverAndCaptureCookies(url) }
    ...
    // pageReadyScriptProvider fires on every poll regardless of readiness, so it's a convenient
    // hook to capture the browser session's cookies as a side effect (returning null injects no
    // script). Android's CookieManager gave the original a shared, process-wide cookie store to
    // read from after the fact; desktop has no such thing, so this is captured directly instead.
    private suspend fun getResolverAndCaptureCookies(url: String): String {
        val result = getResolver().getResult(
            url,
            pageReadyScriptProvider = { _, _, cookies -> storedClearanceCookie = cookies; null }
        )
        return result.html
    }
    ...
    private fun clearanceCookieHeader(requestUrl: String): String? = storedClearanceCookie
