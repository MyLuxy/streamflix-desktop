...
class StreamingCommunityProvider(private val _language: String? = null) : Provider {
    ...
    private var domain: String
        ...
        set(value) {
            val currentDomain = _domain ?: UserPreferences.streamingcommunityDomain?.ifEmpty { DEFAULT_DOMAIN } ?: DEFAULT_DOMAIN
    ...
    private suspend fun resolveFinalBaseUrl(startBaseUrl: String): String = withContext(Dispatchers.IO) {
        try {
    ...
    override suspend fun getMovie(id: String): Movie = coroutineScope {
        ...
        return@coroutineScope Movie(
            ...
            poster = getImageLink(title.images.find { img -> img.type == "poster" }?.filename), banner = getImageLink(title.images.find { img -> img.type == "background" }?.filename), logo = getImageLink(title.images.find { img -> img.type == "logo" }?.filename),
