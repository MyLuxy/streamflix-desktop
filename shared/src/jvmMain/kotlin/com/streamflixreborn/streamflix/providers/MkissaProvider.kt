...
object MkissaProvider : Provider {
    ...
    override suspend fun getHome(): List<Category> = coroutineScope {
        fun category(name: String, block: suspend () -> List<ListItem>) = async {
            Category(
                name = name,
                list = try {
                    block()
...
//            .filterNot { it.isKnownDeadEmbedSource() }
        ...
        return items
            .distinctBy { item ->
                when (item) {
                    is Movie -> item.id
                    is TvShow -> item.id
                    else -> item::class.simpleName ?: item.toString()
                }
            }
            .take(limit)
    }
    ...
    private fun parseSearchItems(response: JSONObject): List<ListItem> {
        ...
        items.forEach { item ->
            when (item) {
                ...
                else -> Log.d(TAG, "  Other: ${item::class.simpleName}")
            }
        }
        return items
    }
    ...
    private fun TvShow.toMovie(): Movie {
        return Movie(
            ...
            title = title,
            overview = overview,
            released = released?.toCalendar()?.let { java.text.SimpleDateFormat("yyyy-MM-dd", java.util.Locale.US).format(it.time) },
            runtime = runtime,
            rating = rating,
            poster = poster,
            banner = banner,
