...
object AnimeUnityProvider : Provider {
    ...
    private fun parseLatestAdditions(document: Element): List<ListItem> {
    ...
    private fun parseFeaturedAnime(document: Element): List<ListItem> {
        val items = mutableListOf<ListItem>()
    ...
    private fun parseAnimeFromJson(records: org.json.JSONArray): List<ListItem> {
    ...
    private suspend fun getAnimeByGenre(genreId: Int, genreName: String, offset: Int = 0): List<ListItem> {
        return try {
            service.getArchivio()
